...
handlers._users.delete = function(data, callback){
  ...
  var phone = typeof(data.queryStringObject.phone) == 'string' &&
    data.queryStringObject.phone.trim().length == 10 ?
    data.queryStringObject.phone.trim() : false;
    //console.log('typeof(phone number) is: ', typeof(data.queryStringObject.phone));
    //console.log('phone number is: ', data.queryStringObject.phone.trim());
    //console.log('phone number length is: ', data.queryStringObject.phone.trim().length);
  if(phone){
    // Get the token from the headers
    var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
    // Verify that the given token is valid for the phone number
...
handlers._tokens.put = function(data, callback){
  ...
  if(id && extend){
    ...
    callback(400, {'Error': 'Missing required fiels(s), of filed(s) are invalid.'});
  }
};
...
handlers._tokens.verifyToken = function(id, phone, callback){
  ...
  _data.read('tokens', id, function(err, tokenData){
    if(!err && tokenData){
      console.log('*** D E B U G *** - tokenData.phone is ', tokenData.phone);
      console.log('*** D E B U G *** - tokenData.expires is ', tokenData.expires);
      // Check that the token is for the givenuser and is not expired
...
// Container for the checks subhandlers/methods
handlers._checks = {};
        ...
        _data.read('users', userPhone, function(err, userData){
          if(!err && userData){
            ...
            if(userChecks.length < config.maxChecks){
              // Create a random id or the check
              var checkId = helpers.createRandomString(20);

              //Create the check object, and include the user's userPhone
              var checkObject = {
                'id' : checkId,
                'userPhone' : userPhone,
                'protocol' : protocol,
              ...
              callback(400, {'Error': 'The user already has the maximun number of checks (' +
                config.maxChecks + ').'});
            }
          } else {
            callback(403);
  ...
  if(id){
    ...
    _data.read('checks', id, function(err, checkData){
      if(!err && checkData){
        ...
        console.log('This is the check data: \n', checkData);
...
// Required data: id
// Optional data: protocol, url, methon, successCodes, timeoutSeconds (at least one of these)
...
handlers._checks.delete = function(data, callback){
  ...
  var id = typeof(data.queryStringObject.id) == 'string' &&
    data.queryStringObject.id.trim().length == 20 ?
    data.queryStringObject.id.trim() : false;
  console.log('typeof(id) is: ', typeof(data.queryStringObject.id));
  //console.log('id is: ', data.queryStringObject.id.trim());
  console.log('id is: ', data.queryStringObject.id);
  //console.log('id length is: ', data.queryStringObject.id.trim().length);
  if(id){
    // Lookup the check
    _data.read('checks', id, function(err, checkData){
      if(!err, checkData){
              ...
              if(!err){
                ...
                _data.read('users', checkData.userPhone, function(err, userData){
                  if(!err && userData){
                    ...
                    // Determine what the user's checks are
                    var userChecks = typeof(userData.checks) == 'object' &&
                      userData.checks instanceof Array ? userData.checks : [];
                    ...
                    if(checkPosition > -1){
                      ...
                      _data.update('users', checkData.userPhone, userData, function(err){
                        ...
                        callback(200);
                          ...
                          callback(500, {'Error': 'Could not update the user\'s check data.'});
                        }
                      });
                    } else {
                        callback(500, {'Error': 'Could find the check on the user\'s object, so coul dnot remove it.'});
                    }
                  } else {
