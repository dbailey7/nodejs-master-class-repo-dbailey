...
workers.init = function(){
  ...
  // Call the loop so the checks will execute ongoingly ...
  workers.loop();
...
// Lookup all checks, get their data, and send off to the validator
workers.gatherAllChecks = function(){
  ...
  _data.list('checks', function(err, checks){
    if(!err && checks && checks.length > 0){
      ...
      debug('Error is ', err);
      debug('Error: Oops, could not find any checks to process.');
    }
  });
};
